    ...
    stages {
         stage('Build') {
            ...
            publishers: [
                ...
                transfers: [
                    sshTransfer(
                        execCommand: " rm -rf /var/www/api.incidents.agentsoncloud.com"
                          ),
                    sshTransfer(
                        execCommand: "cd /var/www/ && git clone --branch release_1 https://github.com/agents-on-cloud/api.incidents.agentsoncloud.com.git"
                          ),
              sshTransfer(
                        execCommand: "cd /var/www/api.incidents.agentsoncloud.com && npm i && pm2 start"
                         ),
                ])
            ])
            }
